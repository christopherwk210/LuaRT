...
void zip_set_last_error(struct zip_t *zip, mz_zip_error err) {
  mz_zip_set_last_error(&zip->archive, err);
}

mz_uint64 zip_getsize(struct zip_t *zip) {
  return zip->archive.m_archive_size;
}

int zip_locatefile(struct zip_t *zip, const char *start) {
  return mz_zip_reader_locate_file(&zip->archive, start, NULL, 0);
}

struct zip_t *zip_mem_new(void *data, size_t size) {
  struct zip_t *z = (struct zip_t *)calloc((size_t)1, sizeof(struct zip_t));
  z->level = MZ_DEFAULT_LEVEL;
  if (mz_zip_reader_init_mem(&z->archive, data, size,  MZ_DEFAULT_LEVEL | MZ_ZIP_FLAG_DO_NOT_SORT_CENTRAL_DIRECTORY) == FALSE) {
		free(z);
		z = NULL;
	}
	return z;
