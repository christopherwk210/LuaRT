...
LUA_METHOD(keyboard, tostring) {
  ...
  while (++i <= nargs) {
    ...
    if ((res = ToUnicode(vk, 0, keys, &buff, 1, 0)) > 0)
  ...
  if (isdead == TRUE) {
    wchar_t buff = 0;
    BYTE keys[256] = {0};
    ToUnicode(lastvk, 0, keys, &buff, 1, 0);
    lua_pushlwstring(L, &buff, 1);
  }
  lua_concat(L, lua_gettop(L)-nargs);
  return 1;
}
...
static int waitkey(lua_State* L, int status, lua_KContext ctx)  {
  ...
  if (!bk->key) {
    ...
    if (!bk->funcref) {
      ...
      lua_pushboolean(L, GetKeyState(VK_MENU) & 0x8000);
